...
export const timetableSessionSchema = z.object({
  ...
  startTime: z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/, { message: 'Format d\'heure de début invalide (HH:mm).' }),
  endTime: z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/, { message: 'Format d\'heure de fin invalide (HH:mm).' }),
  room: z.string().trim().optional().nullable(),
  sessionType: z.string().trim().min(1).default('CM'),
  recurrence: z.string().trim().optional().default('weekly'),
